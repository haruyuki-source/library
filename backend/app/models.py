...
class BookSchema(Schema):
    ...
    author = fields.String(validate=validate.Length(max=128), allow_none=True)
    publisher = fields.String(validate=validate.Length(max=128), allow_none=True)
    publish_year = fields.Integer(validate=validate.Range(min=0, max=3000), allow_none=True)
    category_id = fields.Integer(allow_none=True)
    location = fields.String(validate=validate.Length(max=64), allow_none=True)
    total_quantity = fields.Integer(load_default=1, validate=validate.Range(min=0), allow_none=True)
    available_quantity = fields.Integer(load_default=1, validate=validate.Range(min=0), allow_none=True)
    price = fields.Float(load_default=0.0, validate=validate.Range(min=0), allow_none=True)
    description = fields.String(allow_none=True)
    cover_url = fields.String(validate=validate.Length(max=500), allow_none=True)
    created_at = fields.DateTime(dump_only=True)
