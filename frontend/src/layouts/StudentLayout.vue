...
const menus = computed(() =>
  router.options.routes
    .find((r) => r.path === '/student')
    ?.children?.filter((c) => c.meta?.title)
    .map((c) => ({
      index: `/student/${c.path}`,
      title: c.meta?.title,
      icon: c.meta?.icon
    })) || []
)
...
onMounted(() => {
  // 刷新学生资料,避免 localStorage 缓存的姓名等信息过期
      ...
      <el-header class="header">
        ...
        <el-dropdown @command="handleLogout">
          <span class="user">
            ...
            <span class="user-name" title="个人信息" @click.stop="goProfile">{{ student.user?.name || '学生' }}</span>
