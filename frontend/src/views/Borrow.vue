...
const dialog = reactive({
  visible: false,
  form: { book_id: null, reader_id: null }
})
...
async function submitBorrow() {
  ...
  Object.assign(dialog.form, { book_id: null, reader_id: null })
  fetchList()
}
...
function statusType(status) {
  return { borrowed: 'warning', returned: 'success', overdue: 'danger' }[status] || 'info'
}
...
<template>
  <div>
    ...
    <el-dialog v-model="dialog.visible" title="新增借阅" width="440px">
      <el-form ref="formRef" :model="dialog.form" :rules="rules" label-width="80px">
        <el-form-item label="图书ID" prop="book_id">
          <el-input-number v-model="dialog.form.book_id" :min="1" />
        </el-form-item>
        <el-form-item label="读者ID" prop="reader_id">
          <el-input-number v-model="dialog.form.reader_id" :min="1" />
        </el-form-item>
      </el-form>
      <template #footer>
